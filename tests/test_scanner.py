...
class TestModScanner:
    ...
    def test_scan_with_corrupted_jar(
        self, temp_dir, mock_corrupted_jar, mock_fabric_jar
    ):
        ...
        assert any("security" in err.lower() or "zip" in err.lower() for err in result.errors)
    ...
    def test_progress_callback(self, mock_mods_folder):
        ...
        def callback(update):
            progress_updates.append((update.current, update.total, update.filename))
    ...
    def test_exclude_patterns(self, mock_mods_folder):
        ...
        result = scanner.scan_folder(mock_mods_folder, exclude_patterns=["*fabric*"])
...
class TestScanResult:
    ...
    def test_filter_by_loader(self, mock_mods_folder):
        ...
        fabric_mods = result.filter_by_loader("fabric")
        assert all(m.loader == "fabric" for m in fabric_mods)
        assert len(fabric_mods) >= 1
