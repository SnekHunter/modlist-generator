...
def test_validate_jar_file_too_large(temp_dir):
    ...
    # Create a JAR that exceeds MAX_FILE_SIZE
    # We'll mock the file size check by creating metadata with huge size
    with zipfile.ZipFile(large_jar, 'w') as jar:
        # Add a file with reported size larger than limit
        info = zipfile.ZipInfo("huge_file.bin")
        info.file_size = security.MAX_FILE_SIZE + 1
        info.compress_size = 100  # Actual compressed size is small
        jar.writestr(info, b"small actual content")
...
def test_safe_extract_file_valid(mock_fabric_jar):
    ...
    with zipfile.ZipFile(mock_fabric_jar, 'r') as jar:
        ...
        assert b"fabric_mod_example" in content
...
def test_safe_extract_file_too_large():
    ...
    try:
        ...
        with zipfile.ZipFile(large_jar, 'w') as jar:
            ...
            info.file_size = security.MAX_FILE_SIZE + 1
            jar.writestr(info, b"content")
        
        with zipfile.ZipFile(large_jar, 'r') as jar:
            with pytest.raises(ResourceLimitError):
                safe_extract_file(jar, "large_file.bin")
    finally:
        shutil.rmtree(temp_dir)
