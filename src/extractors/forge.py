...
try:
    ...
    try:
        ...
        logger.warning(
            "TOML parsing not available. Install 'tomli' for full Forge/NeoForge support."
        )
...
class ForgeTomlExtractor(BaseExtractor):
            ...
            if mod_id in dependencies:
                ...
                if isinstance(mod_deps, list):
                    for dep in mod_deps:
                        if (
                            isinstance(dep, dict)
                            and dep.get("modId", "").lower() == "neoforge"
                        ):
                            return "neoforge"
        ...
        if isinstance(deps_section, dict):
            ...
            if isinstance(mod_deps, list):
                for dep in mod_deps:
                    if (
                        isinstance(dep, dict)
                        and dep.get("modId", "").lower() == "minecraft"
                    ):
    ...
    def extract(
        ...
        try:
            ...
            content_bytes = security.safe_extract_file(
                jar, toml_file, max_size=1024 * 1024
            )
            ...
            error = (
                f"Error extracting Forge/NeoForge mod info from {jar_path.name}: {e}"
            )
            logger.error(error)
            return (None, error)

    def _get_version_from_manifest(
        self, jar: zipfile.ZipFile, files: List[str]
    ) -> Optional[str]:
        ...
        try:
            ...
            content_bytes = security.safe_extract_file(
                jar, "META-INF/MANIFEST.MF", max_size=1024 * 1024
            )
            if content_bytes is None:
                return None
...
class LegacyForgeExtractor(BaseExtractor):
    ...
    def extract(
        ...
        try:
            ...
            content_bytes = security.safe_extract_file(
                jar, self.METADATA_FILE, max_size=1024 * 1024
            )
            if content_bytes is None:
            ...
            author = self._normalize_authors(
                mod.get("authorList") or mod.get("authors")
            )
