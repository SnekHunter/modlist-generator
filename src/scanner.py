...
class ModScanner:
    ...
    def _extract_single_mod(
        self, jar_path: Path, disabled: bool = False
            ...
            with zipfile.ZipFile(jar_path, "r") as jar:
                ...
                for extractor in self.extractors:
                    if extractor.can_extract(jar, files):
                        logger.debug(
                            f"Using {extractor.name} extractor for {jar_path.name}"
                        )
    ...
    def scan_folder(
        ...
        if exclude_patterns:
            for pattern in exclude_patterns:
                excluded = set(
                    ...
                    if not recursive
                    else folder_path.rglob(pattern)
                )
        ...
        logger.info(
            f"Found {len(all_files)} JAR file(s). Processing with {self.workers} workers..."
        )
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            future_to_jar = {
                executor.submit(
                    self._extract_single_mod, jar_path, jar_path in disabled_set
                ): jar_path
            ...
            for future in as_completed(future_to_jar):
                ...
                should_update = (
                    (completed - last_progress_count) >= self.progress_batch_size
                    or (time.time() - last_progress_time)
                    >= self.progress_batch_interval
                    or completed == len(all_files)
                )
        ...
        logger.info(
            f"Scan completed in {result.scan_duration:.2f}s. Found {len(result.mods)} mods."
        )
