...
class BaseFormatter(ABC):
    ...
    @abstractmethod
    def format(self, result: ScanResult, include_errors: bool = True, **kwargs: Any) -> str:
...
class JsonFormatter(BaseFormatter):
    ...
    def format(self, result: ScanResult, include_errors: bool = True, **kwargs: Any) -> str:
        ...
        return json.dumps(
            result.to_dict(include_errors), indent=indent, ensure_ascii=False
        )
    ...
    def format(self, result: ScanResult, include_errors: bool = True, **kwargs: Any) -> str:
        ...
        lines.extend(
            [
                "## Mods",
                "",
                "| Name | Loader | Version | Author | MC Version | Status |",
                "|------|--------|---------|--------|------------|--------|",
            ]
        )
        ...
        for mod in result.mods:
            ...
            author = (mod.author or "").replace("|", "\\|")[
                :30
            ]  # Truncate long authors
            mc_ver = (
                ", ".join(mod.mc_versions[:2]) if mod.mc_versions else "-"
            )  # Show first 2
            status = "🔴 Disabled" if mod.disabled else "✅"
            lines.append(
                f"| {name} | {mod.loader} | {mod.version} | {author} | {mc_ver} | {status} |"
            )
...
class YamlFormatter(BaseFormatter):
    ...
    def format(self, result: ScanResult, include_errors: bool = True, **kwargs: Any) -> str:
        ...
        for mod in result.mods:
            ...
            if mod.mc_versions:
                lines.append(
                    f"    mc_versions: [{', '.join(f'\"{v}\"' for v in mod.mc_versions)}]"
                )
            if mod.disabled:
                lines.append("    disabled: true")
            if mod.dependencies:
                lines.append(
                    f"    dependencies: [{', '.join(f'\"{d}\"' for d in mod.dependencies)}]"
                )
