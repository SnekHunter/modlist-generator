    ...
    try:
        ...
        if len(str(resolved)) > MAX_PATH_LENGTH:
            raise PathTraversalError(
                f"Path too long: {len(str(resolved))} > {MAX_PATH_LENGTH}"
            )
        ...
        for forbidden in FORBIDDEN_DIRS:
            if forbidden.exists():
                try:
                    ...
                    raise PathTraversalError(
                        f"Access to system directory forbidden: {forbidden}"
                    )
                except ValueError:
...
def validate_jar_file(jar_path: Path) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if file_size > MAX_FILE_SIZE:
            return (
                False,
                f"File too large: {file_size / (1024*1024):.1f}MB > {MAX_FILE_SIZE / (1024*1024):.1f}MB",
            )
    ...
    try:
        with zipfile.ZipFile(jar_path, "r") as jar:
            ...
            if len(file_list) > MAX_FILES_IN_JAR:
                return (
                    False,
                    f"Too many files in JAR: {len(file_list)} > {MAX_FILES_IN_JAR}",
                )
            ...
            for info in jar.infolist():
                if info.file_size > MAX_FILE_SIZE:
                    return (
                        False,
                        f"Decompressed file too large: {info.filename} ({info.file_size / (1024*1024):.1f}MB)",
                    )
                ...
                if total_size > MAX_DECOMPRESSED_SIZE:
                    return (
                        False,
                        f"Total decompressed size too large: {total_size / (1024*1024):.1f}MB > {MAX_DECOMPRESSED_SIZE / (1024*1024):.1f}MB",
                    )
...
def safe_extract_file(
    jar: zipfile.ZipFile, entry_name: str, max_size: int = MAX_FILE_SIZE
) -> Optional[bytes]:
    ...
    try:
        ...
        if info.file_size > max_size:
            raise ResourceLimitError(
                f"File too large to extract: {info.file_size} > {max_size}"
            )
        ...
        with jar.open(entry_name) as f:
            ...
            while True:
                ...
                if total_read > max_size:
                    raise ResourceLimitError(
                        f"File exceeded size limit during extraction: {total_read} > {max_size}"
                    )
